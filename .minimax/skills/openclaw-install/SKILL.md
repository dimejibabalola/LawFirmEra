...
1. 执行 `openclaw plugins enable minimax-portal-auth` 启用 minimax-portal-auth 插件

2. 执行 OAuth 授权命令（根据操作系统选择对应命令，参见下方代码块）
...
3. 授权成功后，执行 onboard 命令启动 gateway（参见下方代码块）
...
script -q openclaw models auth login --provider minimax-portal --method "<用户提供的method>" --set-default
...
```powershell
openclaw plugins enable minimax-portal-auth
cmd /c "openclaw models auth login --provider minimax-portal --method `"<用户提供的method>`" --set-default"
openclaw onboard --non-interactive --accept-risk --mode local  --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills
```
